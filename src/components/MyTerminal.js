		...
		terminal.pushToStdout(`Processing approval. Check the activity tab for detailed info`)
		  ...
		  <Barcode value={this.state.account} format="EAN13" />
		   ...
		   .then(data => {
            ...
            playa: {
              description: '<p style="color:hotpink;font-size:1.1em">** BeachMediaPlayer! Loads the underground media player and plays the IPFS resource attached to this UPC.  Resources can be video, audio or even an app!  If it is an app, it is community practice to post a github link to the code so that we can compile and run from our own IPFS node to self verify code safety </p>',
              fn: () => {
                this.setState({progressBal: ''});
                this.setState({ isProgressing: true }, () => {
                  const terminal = this.progressTerminal.current
