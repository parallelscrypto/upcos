    ...
    return (
      ...
      <Terminal
        ...
        commands={{
            ...
            swap: {
		    description: '<p style="color:hotpink;font-size:1.1em">** KeGeExperiments is the token used to write [[intel]] to UPC codes.  In order to acquire KeGeExperiments, you must run the `swap` command. This will `swap` Polygon that you have purchased likely from an exchange for KeGeExperiments from our Decentralized Mint.  No KYC or middleman required.  Specify the amount of KeGeExperiments that you would like to exchange for the Polygon in your wallet in wei.  This will trigger a transaction that will mint equiv. KeGeExperiments for Polygon 1:1.  Example: to buy 5 KeGeExperiments type `swap 5000000000000000000`. In other words, this would send 5 Polygon from your wallet for 5 KeGeExperiments from the KeGeExperiments mint.  Visit <a href="upc://000000000010">[[000000000010]]</a> to view a video tutorial on swap</p>',
              fn: (amount) => {
                this.setState({progressBal: ''});
                this.setState({ isProgressing: true }, () => {
                  const terminal = this.progressTerminal.current
		     ...
		     terminal.pushToStdout(`You have just swapped Polygon for KeGeExperiments.  Check your Activity tab below to track the transaction. \n  Type 'bal' to see your new balance! Balances can sometimes take minutes to update.  THANK YOU! ${approval}`)
                         terminal.pushToStdout(`[[/swap]]`);
                     // expected output: "Success!"
                  });
